...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/reports/affine-index-runs/$RUN_STAMP}"
...
TOPOLOGIES_TEXT="${TOPOLOGY_CLASSES:-translation rotated}"
MATERIALS_TEXT="${MATERIAL_CLASSES:-opaque}"
SAMPLES="${SAMPLES:-1}"
RUNS="${RUNS:-3}"
LEAVES="${LEAVES:-16}"
...
CANDIDATES=(direct program-distance program-surface shared-dag affine-index)
...
for topology in "${TOPOLOGIES[@]}"; do
  for reuse in "${REUSE_FACTORS[@]}"; do
    ...
    groups=$((LEAVES / reuse))
    jq --arg topology "$topology" --argjson leaves "$LEAVES" \
      --argjson reuse "$reuse" --argjson groups "$groups" \
      ...
      def center($g):
        [((($g % columns) - ((columns - 1) / 2)) * 1.02),
         ((((($g / columns) | floor) - ((rows - 1) / 2))) * 0.94),
         (((($g * 7) % 3) - 1) * 0.16)];
      def leaf($i):
        if ($i % 2) == 0
        then {op:"sphere",radius:(0.34 + (($i % 7) * 0.021))}
        else {op:"box",value:[(0.29 + (($i % 5) * 0.019)),
                               (0.33 + (($i % 3) * 0.023)),
                               (0.27 + (($i % 4) * 0.021))]} end;
      def grouped_program:
        reduce range(0;$groups) as $g
          ({previous:[0,0,0],operations:[]};
           center($g) as $current |
           (($g % 5) * 0.13 - 0.26) as $angle |
           .operations += [
             {op:"translate",value:[($current[0]-.previous[0]),
                                    ($current[1]-.previous[1]),
                                    ($current[2]-.previous[2])]}
           ] + (if $topology == "rotated" then [{op:"rotate_z",angle:$angle}] else [] end)
             + [range(0;$reuse) as $local | leaf(($g * $reuse) + $local)]
             + (if $topology == "rotated" then [{op:"rotate_z",angle:(-$angle)}] else [] end) |
           .previous = $current) | .operations;
      .preset = $preset |
      .camera.position = [0.0,0.0,-6.6] |
      .camera.focus_distance = 6.6 |
      .camera.fov = 47 |
      .voxel.bounds_min = [-3.25,-3.25,-3.25] |
      .voxel.bounds_max = [3.25,3.25,3.25] |
      .sdf_program.operations = grouped_program
    ' scenes/benchmarks/Exact_Sphere.json > "$OUT_DIR/fixtures/$topology-reuse-$reuse-base.json"
    for material in "${MATERIALS[@]}"; do
      jq --arg topology "$topology" --argjson reuse "$reuse" --arg material "$material" '
        .output = ("Affine_Index_" + $topology + "_" + ($reuse|tostring) + "x_" + $material + ".png") |
        .sdf_program.material.mode = "solid" |
        .sdf_program.material.color =
          (if $material == "opaque" then [0.18,0.64,0.94]
           elif $material == "translucent" then [0.28,0.82,0.62]
           else [0.72,0.88,0.98] end) |
        .sdf_program.material.roughness =
          (if $material == "opaque" then 0.4
           elif $material == "translucent" then 0.22 else 0.05 end) |
        .sdf_program.material.specular = (if $material == "opaque" then 0.3 else 0.9 end) |
      ...
      ' "$OUT_DIR/fixtures/$topology-reuse-$reuse-base.json" \
        > "$OUT_DIR/fixtures/$topology-reuse-$reuse-$material.json"
    done
  done
done
        ...
        for material in "${MATERIALS[@]}"; do
          for candidate in "${CANDIDATES[@]}"; do
            case "$candidate" in
              ...
              affine-index) backend_args=(--sdf-affine-index-topology-specialization) ;;
            esac
            for ((run=1; run<=RUNS; ++run)); do
              run_dir="$OUT_DIR/renders/$resolution/bounce-$bounces/$topology-reuse-$reuse-$material/$candidate/run-$run"
              mkdir -p "$run_dir"
              jq -nc --args '$ARGS.positional' -- \
                "$OUT_DIR/fixtures/$topology-reuse-$reuse-$material.json" --out "$run_dir" \
...
for resolution in "${RESOLUTIONS[@]}"; do
  for bounces in "${BOUNCES[@]}"; do
    for topology in "${TOPOLOGIES[@]}"; do
      for reuse in "${REUSE_FACTORS[@]}"; do
        for material in "${MATERIALS[@]}"; do
          output="Affine_Index_${topology}_${reuse}x_${material}.png"
          for candidate in "${CANDIDATES[@]}"; do
            for ((run=1; run<=RUNS; ++run)); do
              metadata="$OUT_DIR/renders/$resolution/bounce-$bounces/$topology-reuse-$reuse-$material/$candidate/run-$run/$output.render.json"
              jq -c --arg resolution "$resolution" --argjson bounces "$bounces" \
                --arg topology "$topology" --argjson requested_reuse "$reuse" \
                --arg material "$material" --arg candidate "$candidate" --argjson run "$run" '
                {resolution:$resolution,bounces:$bounces,topology:$topology,
                 requested_reuse:$requested_reuse,material:$material,candidate:$candidate,run:$run,
                 elapsed_ms:.elapsed_ms,build_ms:(.sdf_runtime_library_build_ms // 0),
                 evaluator:.sdf_direct_evaluator,source_instructions:.sdf_program_source_instruction_count,
                 ...
                 wide_bytes:.sdf_canonical_wide_parameter_bytes,
                 indexed_bytes:.sdf_canonical_indexed_parameter_bytes,
          ...
          if [[ "$resolution" == "1920x1080" && "$bounces" == "8" ]]; then
            direct_image="$OUT_DIR/renders/$resolution/bounce-$bounces/$topology-reuse-$reuse-$material/direct/run-1/$output"
            for candidate in "${CANDIDATES[@]:1}"; do
              candidate_image="$OUT_DIR/renders/$resolution/bounce-$bounces/$topology-reuse-$reuse-$material/$candidate/run-1/$output"
              "$BIN" compare "$direct_image" "$candidate_image" \
                --report "$OUT_DIR/comparisons/$topology-reuse-$reuse-$material-$candidate.json"
            done
          fi
        done
      done
...
jq -s '
  ...
  group_by([.resolution,.bounces,.topology,.requested_reuse,.material,.candidate]) |
  map(.[0] + {elapsed_ms:(map(.elapsed_ms)|median),build_ms:(map(.build_ms)|median)}) |
  group_by([.resolution,.bounces,.topology,.requested_reuse,.material]) |
  map((map({key:.candidate,value:.})|from_entries) as $c |
      ($c.direct.elapsed_ms) as $direct |
      {resolution:$c.direct.resolution,bounces:$c.direct.bounces,topology:$c.direct.topology,
       ...
       wide_bytes:$c.direct.wide_bytes,indexed_bytes:$c.direct.indexed_bytes,candidates:$c,
       speedup:{program_distance:($direct/$c["program-distance"].elapsed_ms),
                ...
                affine_index:($direct/$c["affine-index"].elapsed_ms)},
       affine_vs_program:($c["program-distance"].elapsed_ms/$c["affine-index"].elapsed_ms),
       winner:($c|to_entries|min_by(.value.elapsed_ms)|.key)}) as $rows |
  {settings:{samples:$ARGS.named.samples,runs:$ARGS.named.runs,leaves:$ARGS.named.leaves},rows:$rows,
   aggregates:{conditions:($rows|length),
     ...
     mean_speedup:{program_distance:($rows|map(.speedup.program_distance)|add/length),
                   ...
                   affine_index:($rows|map(.speedup.affine_index)|add/length)},
     by_topology_reuse:($rows|group_by([.topology,.requested_reuse])|map({
       key:(.[0].topology+"-"+(.[0].requested_reuse|tostring)),
       value:{conditions:length,affine_index_mean:(map(.speedup.affine_index)|add/length),
              program_distance_mean:(map(.speedup.program_distance)|add/length),
              affine_vs_program_mean:(map(.affine_vs_program)|add/length),
              affine_index_wins:(map(select(.winner=="affine-index"))|length),
              indexed_bytes:.[0].indexed_bytes,wide_bytes:.[0].wide_bytes}})|from_entries)}}
...
printf 'Affine-index run experiment: %s\n' "$OUT_DIR"
